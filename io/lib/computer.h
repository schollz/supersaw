...
uint8_t GetBoardID() {
  ...
  sleep_us(1);
  uint8_t pd = gpio_get(BOARD_ID_0) | (gpio_get(BOARD_ID_0) << 2) |
  ...
  sleep_us(1);
  uint8_t pu = (gpio_get(BOARD_ID_0) << 1) | (gpio_get(BOARD_ID_0) << 3) |
...
int ReadEEPROM() {
  ...
  for (uint8_t channel = 0; channel < CAL_MAX_CHANNELS; channel++) {
    int channelOffset =
        bufferIndex + (41 * channel);  // channel 0 = 4, channel 1 = 45
